...
const GH_MOMO_RECIPIENTS = [
  { id: "mtn",    network: "MTN MoMo",     networkShort: "MTN",    number: "0594240246", name: "Abraham Kwadwo Asiamah", icon: "phone_android", color: "#f59e0b", colorLow: "rgba(245,158,11,0.1)", colorMid: "rgba(245,158,11,0.3)" },
   { id: "telecel",    network: "TELECEL CASH",     networkShort: "TELECEL",    number: "0502918371", name: "Iklahs Prempeh", icon: "phone_android", color: "#f59e0b", colorLow: "rgba(245,158,11,0.1)", colorMid: "rgba(245,158,11,0.3)" }
] as const;
type MomoId = typeof GH_MOMO_RECIPIENTS[number]["id"];

/* ─── Bank Transfer Details (Nigeria) ───────────────────────────────────── */
const BANK_NAME        = "BANKIT Microfinance Bank";
const BANK_ACCT_NAME   = "Aisha Abdulmalik";
const BANK_ACCT_NUMBER = "8051691303";
...
/* ─── ImgBB (crypto screenshot hosting) ─────────────────────────────────────
   The backend's BinanceDeposit.screenshotUrl column is capped at 512 chars,
   so a compressed base64 data-URL (tens of thousands of chars) will never
   fit. ImgBB returns a short, real, public HTTPS URL instead — well under
   that limit — which is what gets sent as screenshotUrl on submit.
──────────────────────────────────────────────────────────────────────────── */
...
async function uploadToImgBB(file: File): Promise<string> {
  ...
  if (!url) throw new Error("Image host returned no URL — check the ImgBB API key.");
  return url;
}

/* ─── Design Tokens ─────────────────────────────────────────────────────── */
const T = {
  bg: "#050c1a", surface: "#0a1628", raised: "#0f1f38",
  border: "rgba(99,179,237,0.12)",
  blue: "#2563eb", blueBright: "#3b82f6",
  blueLow: "rgba(37,99,235,0.12)", blueMid: "rgba(37,99,235,0.32)",
  gold: "#f59e0b", goldLow: "rgba(245,158,11,0.1)", goldMid: "rgba(245,158,11,0.32)",
  green: "#22c55e", greenLow: "rgba(34,197,94,0.1)", greenMid: "rgba(34,197,94,0.22)",
  white: "#f0f6ff", dim: "rgba(224,238,255,0.42)", faint: "rgba(224,238,255,0.05)",
};
...
const btnPrimaryDis: React.CSSProperties = {
  ...btnPrimary,
  cursor: "not-allowed",
  opacity: 0.38,
};
...
/* ── Screenshot Upload widget (reused — MoMo / Bank NG, base64) ── */
function ScreenshotUpload({ screenshot, compressing, errs, onFileChange, onRemove }: {
  screenshot: string; compressing: boolean;
  errs: Record<string,string>; onFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void; onRemove: () => void;
}) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {screenshot ? (
        <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", border: `1px solid ${T.blueMid}`, background: T.bg }}>
          <img src={screenshot} alt="Payment screenshot" style={{ width: "100%", maxHeight: 200, objectFit: "contain", display: "block" }} />
          {compressing && <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}><Spin /></div>}
          {!compressing && (
            <>
              <div style={{ position: "absolute", top: 8, right: 8, display: "flex", gap: 6 }}>
                ...
                <button onClick={onRemove} style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 10, fontWeight: 700, padding: "4px 9px", borderRadius: 6, cursor: "pointer", border: "none", background: "rgba(239,68,68,0.7)", color: "#fff", fontFamily: "inherit" }}>
        ...
        <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 100, border: `2px dashed ${errs.screenshot ? "rgba(239,68,68,0.5)" : T.border}`, borderRadius: 10, cursor: compressing ? "wait" : "pointer", background: T.faint, transition: "all 0.2s" }}>
          {compressing ? <><Spin /><span style={{ fontSize: 11, color: T.dim, marginTop: 8 }}>Processing…</span></> : <>
      ...
      {errs.screenshot && <div style={{ fontSize: 11, color: "#f87171", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 12 }}>error</span>{errs.screenshot}</div>}
      {!errs.screenshot && !screenshot && <div style={{ fontSize: 11, color: T.dim, marginTop: 4 }}>Upload a screenshot of your payment confirmation.</div>}
      {!errs.screenshot && screenshot && <div style={{ fontSize: 11, color: T.green, marginTop: 4, display: "flex", alignItems: "center", gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 13 }}>check_circle</span>Screenshot attached — will be sent with your deposit proof</div>}
    </div>
  );
}

/* ── Crypto Screenshot Upload widget (NEW — Binance/crypto only, ImgBB-hosted) ──
   Screenshot is OPTIONAL (recommended), not required, for the crypto flow.
   Picking a file shows an instant local preview while it uploads to ImgBB
   in the background; the short hosted URL becomes `screenshot`.
──────────────────────────────────────────────────────────────────────────── */
function CryptoScreenshotUpload({ screenshot, preview, uploading, error, onFileChange, onRemove }: {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={lbl}>
        Payment Screenshot{" "}
        <span style={{ color: T.dim, textTransform: "none", fontSize: 10 }}>(recommended)</span>
      </label>
      ...
      <button
        ...
        style={{
          ...
          padding: preview ? 0 : "22px 16px", borderRadius: 10,
          cursor: uploading ? "wait" : "pointer", boxSizing: "border-box", overflow: "hidden",
          backgroundColor: screenshot ? T.greenLow : T.faint,
          border: `2px dashed ${error ? "rgba(239,68,68,0.5)" : screenshot ? T.greenMid : T.border}`,
          color: screenshot ? T.green : T.dim,
          display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8,
          fontFamily: "inherit",
        }}
      >
        {preview ? (
          <img src={preview} alt="Payment proof" style={{ width: "100%", maxHeight: 160, objectFit: "contain", opacity: uploading ? 0.5 : 1, display: "block" }} />
        ) : (
          <>
            <span className="material-symbols-outlined" style={{ fontSize: 28, color: T.dim }}>add_photo_alternate</span>
            <span style={{ fontSize: 12, fontWeight: 600 }}>Tap to upload screenshot</span>
            <span style={{ fontSize: 10, color: T.dim }}>PNG · JPG · WEBP — max 10 MB</span>
          </>
        )}
        {uploading && (
      ...
      {screenshot && !uploading && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
          <div style={{ fontSize: 11, color: T.green, display: "flex", alignItems: "center", gap: 4 }}>
            <span className="material-symbols-outlined" style={{ fontSize: 13 }}>check_circle</span>Uploaded — tap image to change
          </div>
          <button onClick={onRemove} style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 6, cursor: "pointer", border: "none", background: "rgba(239,68,68,0.15)", color: "#f87171", fontFamily: "inherit" }}>
            Remove
          </button>
        </div>
      )}
      {error && <div style={{ fontSize: 11, color: "#f87171", marginTop: 5, display: "flex", alignItems: "center", gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 12 }}>error</span>{error}</div>}
      {!screenshot && !uploading && !error && <div style={{ fontSize: 11, color: T.dim, marginTop: 4 }}>Optional but speeds up verification.</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {[{ label: "MoMo", matIcon: "phone_android", desc: "Ghana" }, { label: "Bank", matIcon: "account_balance", desc: "Transfer" }, { label: "Binance", matIcon: "currency_bitcoin", desc: "Crypto" }].map(b => (
...
function CountryDropdown({ country, ipDetecting, onSelect }: { country: Country | null; ipDetecting: boolean; onSelect: (c: Country) => void }) {
      ...
      {dropOpen && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, zIndex: 100, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, boxShadow: "0 12px 40px rgba(0,0,0,0.7)", overflow: "hidden" }}>
          ...
          <div style={{ maxHeight: 260, overflowY: "auto" }}>
            {filtered.map(c => {
              const hasMomo = c.gateways.includes("bank_gh"), hasBank = c.gateways.includes("bank_ng");
              const badge = hasMomo ? { label: "MOMO", bg: T.blueLow, color: T.blueBright, border: T.blueMid } : hasBank ? { label: "BANK", bg: T.greenLow, color: T.green, border: T.greenMid } : { label: "CRYPTO", bg: T.goldLow, color: T.gold, border: "rgba(245,158,11,0.3)" };
              return (
                <button key={c.code} onClick={() => { onSelect(c); setDropOpen(false); setSearch(""); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: country?.code === c.code ? T.blueLow : "none", border: "none", borderBottom: `1px solid ${T.border}`, cursor: "pointer", fontFamily: "inherit", transition: "background 0.1s" }}>
                  ...
                  <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 8px", borderRadius: 20, letterSpacing: "0.05em", background: badge.bg, color: badge.color, border: `1px solid ${badge.border}` }}>{badge.label}</span>
                </button>
              );
            })}
          </div>
...
function GatewayTabs({ country, gateway, onSelect }: { country: Country; gateway: "bank_gh" | "binance" | "bank_ng" | null; onSelect: (gw: "bank_gh" | "binance" | "bank_ng") => void }) {
  ...
  const allTabs: TabDef[] = [
    { id: "bank_gh", matIcon: "phone_android",   label: "Mobile Money",  sub: "MTN · Telecel · Ghana" },
    { id: "bank_ng", matIcon: "account_balance",  label: "Bank Transfer", sub: "BANKIT · Nigeria" },
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${tabs.length}, 1fr)`, gap: 8 }}>
        {tabs.map(t => {
          const active = gateway === t.id, isCrypto = t.id === "binance", isBank = t.id === "bank_ng";
          ...
          return (
            <button key={t.id} onClick={() => onSelect(t.id)} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 4, padding: "13px 14px", background: active ? accentBg : T.raised, border: `1.5px solid ${active ? accentBd : T.border}`, borderRadius: 10, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}>
              ...
              {active && <span style={{ fontSize: 9, fontWeight: 800, color: accentClr, marginTop: 2, display: "flex", alignItems: "center", gap: 3 }}><span className="material-symbols-outlined" style={{ fontSize: 11 }}>check_circle</span>SELECTED</span>}
            </button>
          );
        })}
      </div>
...
function SupportPanel() {
  ...
  return (
    <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden", marginTop: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              ...
              <a key={ch.label} href={ch.href} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 12, background: T.raised, border: `1px solid ${T.border}`, borderRadius: 10, padding: "11px 13px", textDecoration: "none" }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 12, fontWeight: 700, color: T.white }}>{ch.label}</div><div style={{ fontSize: 11, color: T.dim }}>{ch.desc}</div></div>
...
/* ── Ghana MoMo Info — two recipient picker ── */
function GhMomoInfo({ error, onNext }: { error: string; onNext: (r: MomoId) => void }) {
  const [selected, setSelected] = useState<MomoId | null>(null);
  return (
    <div>
      {error && <ErrBox msg={error} />}
      <div style={{ background: T.blueLow, border: `1px solid ${T.blueMid}`, borderRadius: 9, padding: "9px 13px", marginBottom: 16, fontSize: 12, color: T.blueBright, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <label style={lbl}>Choose MoMo recipient</label>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        {GH_MOMO_RECIPIENTS.map(r => {
          const active = selected === r.id;
          return (
            <button key={r.id} onClick={() => setSelected(r.id as MomoId)}
              style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 6, padding: "14px 13px", background: active ? r.colorLow : T.raised, border: `1.5px solid ${active ? r.color : T.border}`, borderRadius: 12, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s", textAlign: "left" }}>
              <span className="material-symbols-outlined" style={{ fontSize: 26, color: active ? r.color : T.dim }}>{r.icon}</span>
              <span style={{ fontSize: 12, fontWeight: 800, color: T.white, lineHeight: 1.2 }}>{r.network}</span>
              <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 13, fontWeight: 700, color: active ? r.color : T.dim, letterSpacing: 1 }}>{r.number}</span>
              <span style={{ fontSize: 10, color: T.dim, lineHeight: 1.4 }}>{r.name}</span>
              {active && <span style={{ fontSize: 9, fontWeight: 800, color: r.color, display: "flex", alignItems: "center", gap: 3, marginTop: 2 }}><span className="material-symbols-outlined" style={{ fontSize: 11 }}>check_circle</span>SELECTED</span>}
            </button>
          );
        })}
      </div>
      ...
      {selected && (() => {
        const r = GH_MOMO_RECIPIENTS.find(x => x.id === selected)!;
        return (
          <div style={{ background: T.raised, border: `1px solid ${r.colorMid}`, borderRadius: 12, padding: 16, marginBottom: 14, animation: "_fadeUp 0.2s ease" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <div style={{ width: 34, height: 34, borderRadius: 8, background: r.colorLow, border: `1px solid ${r.colorMid}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span className="material-symbols-outlined" style={{ color: r.color, fontSize: 18 }}>{r.icon}</span>
              </div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13, color: T.white }}>Send {r.networkShort} MoMo to this number</div>
                <div style={{ fontSize: 11, color: T.dim }}>Then submit your payment proof</div>
              </div>
            </div>
            {[
              { icon: "corporate_fare", label: "Network",      value: r.network, mono: false },
              { icon: "person",         label: "Account Name", value: r.name,    mono: false },
              { icon: "phone",          label: "Phone Number", value: r.number,  mono: true  },
            ].map(row => (
            ...
            <div style={{ background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.22)", borderRadius: 8, padding: "9px 12px", fontSize: 11, color: T.gold, lineHeight: 1.6, display: "flex", gap: 7 }}>
              ...
              Always include your <strong>username or phone number</strong> in the MoMo reference so we can identify your payment.
            </div>
          </div>
        );
      })()}
      ...
      <button onClick={() => selected && onNext(selected)} disabled={!selected}
        style={{ ...btnPrimary, marginBottom: 8, opacity: selected ? 1 : 0.4 }}>
        <span className="material-symbols-outlined" style={{ fontSize: 18 }}>task_alt</span>
        {selected ? "I've Sent — Submit Proof" : "Select a recipient above"}
      </button>
      <div style={{ textAlign: "center", fontSize: 11, color: T.dim, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
        <span className="material-symbols-outlined" style={{ fontSize: 13 }}>manage_search</span>Verified within 5–15 minutes
      </div>
    </div>
  );
}

/* ── Ghana MoMo Proof Form ── */
function GhMomoForm({ error, momoRecipient, bankRef, setBankRef, bankAmtSent, setBankAmtSent, bankExpected, setBankExpected, bankSender, setBankSender, bankNote, setBankNote, bankScreenshot, setBankScreenshot, bankCompressing, bankErrs, setBankErrs, loading, onFileChange, onSubmit, onBack }: {
  error: string; momoRecipient: MomoId | null;
  ...
  loading: boolean; onFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void; onSubmit: () => void; onBack: () => void;
}) {
  const fe = (k: string) => bankErrs[k] ? <div style={{ fontSize: 11, color: "#f87171", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 12 }}>error</span>{bankErrs[k]}</div> : null;
  ...
  const r = momoRecipient ? GH_MOMO_RECIPIENTS.find(x => x.id === momoRecipient) : null;
  return (
    <div>
      {error && <ErrBox msg={error} />}
      {r && (
        <div style={{ background: r.colorLow, border: `1px solid ${r.colorMid}`, borderRadius: 9, padding: "9px 13px", marginBottom: 14, fontSize: 12, color: r.color, display: "flex", alignItems: "center", gap: 8 }}>
          <span className="material-symbols-outlined" style={{ fontSize: 15, flexShrink: 0 }}>{r.icon}</span>
          Paying via <strong>{r.network}</strong> to <strong>{r.number}</strong>
        </div>
      )}
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", background: T.raised, border: `1px solid ${bankErrs.sender ? "rgba(239,68,68,0.5)" : T.border}`, borderRadius: 10, overflow: "hidden" }}>
          ...
          <input type="tel" value={bankSender} onChange={e => { setBankSender(e.target.value); setBankErrs(p => ({ ...p, sender: "" })); }} placeholder="e.g. 0244123456" maxLength={15} style={{ ...inp, border: "none", borderRadius: 0, background: "none" }} />
        </div>
        {bankErrs.sender ? <div style={{ fontSize: 11, color: "#f87171", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 12 }}>error</span>{bankErrs.sender}</div> : <div style={{ fontSize: 11, color: T.dim, marginTop: 4 }}>The MoMo number you're sending from.</div>}
      </div>
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", background: T.raised, border: `1px solid ${bankErrs.ref ? "rgba(239,68,68,0.5)" : T.border}`, borderRadius: 10, overflow: "hidden" }}>
          ...
          <input type="text" value={bankRef} onChange={e => { setBankRef(e.target.value); setBankErrs(p => ({ ...p, ref: "" })); }} placeholder="Your username or reference you used" style={{ ...inp, border: "none", borderRadius: 0, background: "none" }} />
        </div>
        {fe("ref")}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div>
          ...
          <input type="number" value={bankAmtSent} placeholder={`Min GH₵${MIN_DEPOSIT_GHS}`} onChange={e => { setBankAmtSent(e.target.value); setBankErrs(p => ({ ...p, amt: "" })); }} style={fi("amt")} />
          {fe("amt")}
        </div>
        <div>
          <label style={lbl}>Expected GH₵ Credit <span style={{ color: "#f87171" }}>*</span></label>
          <input type="number" value={bankExpected} placeholder="0.00" onChange={e => { setBankExpected(e.target.value); setBankErrs(p => ({ ...p, exp: "" })); }} style={fi("exp")} />
          {fe("exp")}
        </div>
      </div>
      ...
      <ScreenshotUpload screenshot={bankScreenshot} compressing={bankCompressing} errs={bankErrs} onFileChange={onFileChange} onRemove={() => setBankScreenshot("")} />
      <div style={{ marginBottom: 18 }}>
        <label style={lbl}>Note to Admin <span style={{ color: T.dim, textTransform: "none", fontSize: 10 }}>(optional)</span></label>
        <textarea value={bankNote} onChange={e => setBankNote(e.target.value)} placeholder="Any extra info" rows={3} style={{ ...inp, resize: "vertical", lineHeight: 1.6 } as React.CSSProperties} />
      </div>
      <button onClick={onSubmit} disabled={loading || bankCompressing} style={{ ...btnPrimary, opacity: loading || bankCompressing ? 0.38 : 1, marginBottom: 8 }}>
...
function BankNgInfo({ error, onNext }: { error: string; onNext: () => void }) {
  return (
    <div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: T.dim, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
        <span className="material-symbols-outlined" style={{ fontSize: 13 }}>manage_search</span>Verified within 5–10 minutes
      </div>
    </div>
  );
}
...
/* ── Bank NG Form ── */
function BankNgForm({ error, bankRef, setBankRef, bankAmtSent, setBankAmtSent, bankExpected, setBankExpected, bankSender, setBankSender, bankNote, setBankNote, bankScreenshot, setBankScreenshot, bankCompressing, bankErrs, setBankErrs, loading, onFileChange, onSubmit, onBack }: {
  error: string;
  bankRef: string; setBankRef: (v: string) => void;
  bankAmtSent: string; setBankAmtSent: (v: string) => void;
  bankExpected: string; setBankExpected: (v: string) => void;
  ...
  const fe = (k: string) => bankErrs[k] ? <div style={{ fontSize: 11, color: "#f87171", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 12 }}>error</span>{bankErrs[k]}</div> : null;
  const fi = (k: string): React.CSSProperties => ({ ...inp, border: `1px solid ${bankErrs[k] ? "rgba(239,68,68,0.5)" : T.border}` });
  const QUICK = [5000, 10000, 20000, 50000, 100000, 200000];
  return (
    <div>
      {error && <ErrBox msg={error} />}
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", background: T.raised, border: `1px solid ${bankErrs.ref ? "rgba(239,68,68,0.5)" : T.border}`, borderRadius: 10, overflow: "hidden" }}>
          <span className="material-symbols-outlined" style={{ fontSize: 18, color: T.dim, padding: "0 12px", borderRight: `1px solid ${T.border}`, display: "flex", alignItems: "center", height: "100%" }}>tag</span>
          <input type="text" value={bankRef} onChange={e => { setBankRef(e.target.value); setBankErrs(p => ({ ...p, ref: "" })); }} placeholder="Your name, username, or receipt reference" style={{ ...inp, border: "none", borderRadius: 0, background: "none" }} />
        </div>
        {fe("ref")}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div>
          ...
          <input type="number" value={bankAmtSent} placeholder={`Min ₦${MIN_DEPOSIT_NGN.toLocaleString()}`} onChange={e => { setBankAmtSent(e.target.value); setBankErrs(p => ({ ...p, amt: "" })); }} style={fi("amt")} />
          {fe("amt")}
        </div>
        <div>
      ...
      <button onClick={onSubmit} disabled={loading || bankCompressing} style={{ ...btnGreen, opacity: loading || bankCompressing ? 0.38 : 1, marginBottom: 8 }}>
    ...
    <div>
      ...
      <div style={{ background: T.faint, border: `1px solid ${T.border}`, borderRadius: 10, padding: "11px 14px", marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {CRYPTO_COINS.map(c => <span key={c} style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: T.goldLow, color: T.gold, border: "1px solid rgba(245,158,11,0.25)" }}>{c}</span>)}
        </div>
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: T.dim, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
        <span className="material-symbols-outlined" style={{ fontSize: 13 }}>manage_search</span>Reviewed &amp; credited within 1–5 mins
      </div>
    </div>
  );
}

/* ── Binance Proof — UPDATED: now includes the optional ImgBB screenshot
   uploader between "Sender Wallet" and "Note to Admin", matching the
   other two apps. Submit button is disabled while an upload is in flight. ── */
interface BinanceProofProps {
  error: string; txid: string; setTxid: (v: string) => void; cryptoAmt: string; setCryptoAmt: (v: string) => void;
  coin: string; setCoin: (v: string) => void; cryptoNet: string; setCryptoNet: (v: string) => void;
  expectedGhs: string; setExpectedGhs: (v: string) => void; senderAddr: string; setSenderAddr: (v: string) => void;
  userNote: string; setUserNote: (v: string) => void;
  screenshot: string; setScreenshot: (v: string) => void;
  preview: string; setPreview: (v: string) => void;
  uploading: boolean; setUploading: (v: boolean) => void;
  bErrs: Record<string,string>;
  setBErrs: (fn: (p: Record<string,string>) => Record<string,string>) => void;
...
function BinanceProof({
  ...
  screenshot, setScreenshot, preview, setPreview, uploading, setUploading,
  ...
  const handlePick = async (file: File) => {
    ...
    try {
      ...
      setBErrs(p => ({ ...p, screenshot: err instanceof Error ? err.message : "Upload failed. Try again." }));
      setScreenshot("");
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        ...
        <div>
          <label style={lbl}>Expected GH₵ Credit <span style={{ color: "#f87171" }}>*</span></label>
          <input type="number" value={expectedGhs} placeholder="0.00" min="0" step="any" onChange={e => { setExpectedGhs(e.target.value); setBErrs(p => ({ ...p, expectedGhs: "" })); }} style={fi("expectedGhs")} />
          {fe("expectedGhs")}
        </div>
      </div>
      <div style={{ marginBottom: 14 }}>
      ...
      {/* ── NEW: optional ImgBB-hosted screenshot uploader ── */}
      <CryptoScreenshotUpload
      ...
      <div style={{ marginBottom: 18 }}>
        <label style={lbl}>Note to Admin <span style={{ color: T.dim, textTransform: "none", fontSize: 10 }}>(optional)</span></label>
        <textarea value={userNote} onChange={e => setUserNote(e.target.value)} placeholder="Any extra info" rows={3} style={{ ...inp, resize: "vertical", lineHeight: 1.6 } as React.CSSProperties} />
      </div>
      <button onClick={onSubmit} disabled={loading || uploading} style={loading || uploading ? btnPrimaryDis : { ...btnPrimary, marginBottom: 8 }}>
        {loading ? <><Spin /> Submitting…</> : uploading ? <><Spin /> Uploading screenshot…</> : <><span className="material-symbols-outlined" style={{ fontSize: 18 }}>upload_file</span>Submit Deposit Proof</>}
      </button>
      <button onClick={onBack} style={btnGhost}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>arrow_back</span>Back</button>
    </div>
  );
  ...
  return (
    <div style={{ textAlign: "center", padding: "12px 0 8px" }}>
      ...
      <div style={{ fontSize: 13, color: T.dim, lineHeight: 1.7, marginBottom: 22 }}>
        Your {label} is under review.<br />Admin will verify and credit your wallet within <strong style={{ color: T.white }}>{mins}</strong>.
      </div>
      <button onClick={onHome} style={{ ...btnPrimary, marginBottom: 8 }}><span className="material-symbols-outlined" style={{ fontSize: 18 }}>home</span>Back to Home</button>
      <button onClick={onReset} style={btnGhost}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>add_circle</span>Make Another Deposit</button>
    </div>
  );
}

/* ── Crypto Success — now shows a small receipt incl. screenshot status ── */
function CryptoSuccess({ coin, cryptoNet, cryptoAmt, expectedGhs, txid, screenshot, onHome, onReset }: {
  coin: string; cryptoNet: string; cryptoAmt: string; expectedGhs: string; txid: string; screenshot: string;
  ...
  return (
    <div style={{ textAlign: "center", padding: "12px 0 8px" }}>
      ...
      <div style={{ fontSize: 13, color: T.dim, lineHeight: 1.65, marginBottom: 20 }}>
        Your crypto deposit is under review.<br />Admin will credit your Zynobet wallet within <strong style={{ color: T.white }}>1–5 minutes</strong>.
      </div>

      <div style={{ background: T.goldLow, border: `1px solid ${T.goldMid}`, borderRadius: 12, padding: "14px 16px", marginBottom: 20, textAlign: "left" }}>
        {[
          ...
          <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, gap: 10 }}>
            ...
            <span style={{ color: k === "Screenshot" && screenshot ? T.green : T.white, fontSize: 12, fontWeight: 700, textAlign: "right", wordBreak: "break-all", maxWidth: "60%" }}>{v}</span>
          </div>
        ))}
      </div>
...
export default function DepositPage() {
  ...
  const [momoRecipient, setMomoRecipient] = useState<MomoId | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      } catch {} finally { setIpDetecting(false); }
    })();
  }, []);

  useEffect(() => {
    fetch("https://open.er-api.com/v6/latest/GHS").then(r => r.json()).then(d => { if (d?.rates) setRates(d.rates); }).catch(() => {});
  }, []);
  ...
  const [step, setStep] = useState<"form"|"proof"|"success"|"gh_info"|"gh_form"|"gh_success"|"bank_info"|"bank_form"|"bank_success">("form");

  const [txid, setTxid] = useState(""); const [cryptoAmt, setCryptoAmt] = useState(""); const [coin, setCoin] = useState("USDT");
  const [cryptoNet, setCryptoNet] = useState("TRC20"); const [expectedGhs, setExpectedGhs] = useState("");
  const [senderAddr, setSenderAddr] = useState(""); const [userNote, setUserNote] = useState("");
  const [bErrs, setBErrs] = useState<Record<string,string>>({});
  // Crypto screenshot — optional. cryptoShot holds the short hosted ImgBB
  // URL sent to the API; cryptoPreview is a local object-URL shown while
  // the real upload is in flight.
  const [cryptoShot, setCryptoShot] = useState("");
  const [cryptoPreview, setCryptoPreview] = useState("");
  const [cryptoUploading, setCryptoUploading] = useState(false);

  const [bankRef, setBankRef] = useState(""); const [bankAmtSent, setBankAmtSent] = useState("");
  const [bankExpected, setBankExpected] = useState(""); const [bankSender, setBankSender] = useState("");
  const [bankNote, setBankNote] = useState(""); const [bankScreenshot, setBankScreenshot] = useState("");
  const [bankCompressing, setBankCompressing] = useState(false); const [bankErrs, setBankErrs] = useState<Record<string,string>>({});
  ...
  const handleSelectCountry = useCallback((c: Country) => {
    ...
    if (c.gateways.length === 1) {
      ...
      if (gw === "bank_gh") setStep("gh_info"); else if (gw === "bank_ng") setStep("bank_info");
    }
  }, []);

  const selectGateway = useCallback((gw: "bank_gh"|"binance"|"bank_ng") => {
    setGateway(gw); setError("");
    if (gw === "bank_gh") setStep("gh_info"); else if (gw === "bank_ng") setStep("bank_info"); else setStep("form");
  }, []);

  /* ── Binance: validate (screenshot NOT required, but blocks submit while uploading) ── */
  const validateBinance = () => {
    ...
    if (cryptoUploading) e.screenshot = "Please wait for the screenshot to finish uploading";
    setBErrs(e); return Object.keys(e).length === 0;
  };

  /* ── Binance: submit ───────────────────────────────────────────────────────
     FIXED: endpoint corrected from the non-existent
     `/api/wallet/deposit/binance/submit` to the actual controller route
     `/api/wallet/binance-deposits` (same as the other apps), and
     `screenshotUrl` is now included from the ImgBB upload.
  ─────────────────────────────────────────────────────────────────────────── */
  const handleBinanceSubmit = async () => {
    if (!validateBinance()) return; setLoading(true); setError("");
    try {
      await post("/api/wallet/binance-deposits", {
        ...
        screenshotUrl: cryptoShot || undefined,
  ...
  const handleScreenshot = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return; setBankCompressing(true);
    try { const d = await compressImageToBase64(file); setBankScreenshot(d); setBankErrs(p => ({ ...p, screenshot: "" })); }
  ...
  const handleGhMomoSubmit = async () => {
    if (!validateGhMomo()) return; setLoading(true); setError("");
    try {
      const r = GH_MOMO_RECIPIENTS.find(x => x.id === momoRecipient);
      await post("/api/wallet/bank-deposits", {
        transferReference: bankRef.trim(), ngnAmountSent: parseFloat(bankAmtSent),
        expectedNgnCredit: parseFloat(bankExpected), senderAccountName: bankSender.trim(),
        screenshotUrl: bankScreenshot,
        userNote: `[MoMo via ${r?.networkShort ?? momoRecipient}] ${bankNote.trim()}`.trim(),
      });
      setStep("gh_success");
    } catch (e: unknown) { setError((e as Error).message); } finally { setLoading(false); }
  };
  ...
  const handleBankSubmit = async () => {
    if (!validateBank()) return; setLoading(true); setError("");
    try {
      await post("/api/wallet/bank-deposits", { transferReference: bankRef.trim(), ngnAmountSent: parseFloat(bankAmtSent), expectedNgnCredit: parseFloat(bankExpected), senderAccountName: bankSender.trim() || undefined, screenshotUrl: bankScreenshot, userNote: bankNote.trim() || undefined });
      setStep("bank_success");
    } catch (e: unknown) { setError((e as Error).message); } finally { setLoading(false); }
  };

  const resetBankState = useCallback(() => {
    setBankRef(""); setBankAmtSent(""); setBankExpected(""); setBankSender(""); setBankNote(""); setBankScreenshot(""); setBankErrs({});
  }, []);

  const reset = useCallback(() => {
    setCountry(null); setGateway(null); setError(""); setMomoRecipient(null);
    setTxid(""); setCryptoAmt(""); setCoin("USDT"); setCryptoNet("TRC20"); setExpectedGhs(""); setSenderAddr(""); setUserNote(""); setBErrs({});
    setCryptoShot(""); setCryptoPreview(""); setCryptoUploading(false);
    resetBankState(); setStep("form");
  }, [resetBankState]);

  const panelTitle = () => {
    if (!gateway) return null;
    if (gateway === "bank_gh") { if (step === "gh_form") return "Payment Proof"; if (step === "gh_success") return "Under Review"; return "Ghana MoMo"; }
    if (gateway === "binance") { if (step === "proof") return "Payment Proof"; if (step === "success") return "Under Review"; return "Crypto Deposit"; }
    if (gateway === "bank_ng") { if (step === "bank_form") return "Payment Proof"; if (step === "bank_success") return "Under Review"; return "Bank Transfer · NG"; }
    return null;
  };
  ...
  const renderPanel = () => {
    ...
    if (gateway === "bank_gh") {
      if (step === "gh_info") return <GhMomoInfo error={error} onNext={(r) => { setMomoRecipient(r); resetBankState(); setStep("gh_form"); }} />;
      if (step === "gh_form") return <GhMomoForm error={error} momoRecipient={momoRecipient} bankRef={bankRef} setBankRef={setBankRef} bankAmtSent={bankAmtSent} setBankAmtSent={setBankAmtSent} bankExpected={bankExpected} setBankExpected={setBankExpected} bankSender={bankSender} setBankSender={setBankSender} bankNote={bankNote} setBankNote={setBankNote} bankScreenshot={bankScreenshot} setBankScreenshot={setBankScreenshot} bankCompressing={bankCompressing} bankErrs={bankErrs} setBankErrs={setBankErrs} loading={loading} onFileChange={handleScreenshot} onSubmit={handleGhMomoSubmit} onBack={() => setStep("gh_info")} />;
      if (step === "gh_success") return <PendingSuccess label="MoMo payment" mins="5–15 minutes" onHome={() => window.location.href = "/"} onReset={reset} />;
    }
    if (gateway === "binance") {
      if (step === "proof") return (
        <BinanceProof
          ...
          screenshot={cryptoShot} setScreenshot={setCryptoShot}
          preview={cryptoPreview} setPreview={setCryptoPreview}
          uploading={cryptoUploading} setUploading={setCryptoUploading}
      ...
      if (step === "success") return (
        <CryptoSuccess
          ...
          txid={txid} screenshot={cryptoShot}
    ...
    if (gateway === "bank_ng") {
      if (step === "bank_form") return <BankNgForm error={error} bankRef={bankRef} setBankRef={setBankRef} bankAmtSent={bankAmtSent} setBankAmtSent={setBankAmtSent} bankExpected={bankExpected} setBankExpected={setBankExpected} bankSender={bankSender} setBankSender={setBankSender} bankNote={bankNote} setBankNote={setBankNote} bankScreenshot={bankScreenshot} setBankScreenshot={setBankScreenshot} bankCompressing={bankCompressing} bankErrs={bankErrs} setBankErrs={setBankErrs} loading={loading} onFileChange={handleScreenshot} onSubmit={handleBankSubmit} onBack={() => setStep("bank_info")} />;
      if (step === "bank_success") return <PendingSuccess label="bank transfer" mins="5–10 minutes" onHome={() => window.location.href = "/"} onReset={reset} />;
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        ::-webkit-scrollbar{width:4px;}::-webkit-scrollbar-track{background:transparent;}::-webkit-scrollbar-thumb{background:rgba(99,179,237,0.18);border-radius:2px;}
        a:hover{opacity:0.85;}button:hover:not(:disabled){opacity:0.88;}
      `}</style>
        ...
        <div style={{ width: "100%", maxWidth: 420 }}>
          <div style={{ marginBottom: 20, animation: "_fadeUp 0.4s ease" }}>
            ...
            <div style={{ marginTop: 6, fontSize: 12, color: T.dim }}>
              ...
              {country && country.currency !== "GHS" && rates[country.currency] ? <> ≈ <span style={{ color: T.white, fontWeight: 600 }}>{country.symbol}{(MIN_DEPOSIT_GHS * rateFor(country.currency)).toFixed(2)}</span> {country.currency}</> : " — converted to your currency"}
            </div>
          </div>
          <div style={{ animation: "_fadeUp 0.45s ease" }}><TrustBadges /></div>
          <div style={{ background: T.surface, borderRadius: 16, overflow: "visible", border: `1px solid ${T.border}`, boxShadow: "0 20px 60px rgba(0,8,30,0.6)", animation: "_fadeUp 0.5s ease" }}>
            <div style={{ padding: "20px 20px 24px" }}>
              <CountryDropdown country={country} ipDetecting={ipDetecting} onSelect={handleSelectCountry} />
              {country && country.gateways.length > 1 && <GatewayTabs country={country} gateway={gateway} onSelect={selectGateway} />}
              {country && gateway && (
                <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ borderTop: `1px solid ${T.border}`, padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
              <span style={{ fontSize: 11, color: "rgba(224,238,255,0.2)", display: "flex", alignItems: "center", gap: 5 }}><span className="material-symbols-outlined" style={{ fontSize: 14 }}>lock</span>256-bit encrypted · Zynobet</span>
              <span style={{ fontSize: 10, color: "rgba(224,238,255,0.14)" }}>MoMo · Bank · Crypto</span>
            </div>
          </div>
          <SupportPanel />
          <div style={{ marginTop: 20, textAlign: "center", fontSize: 11, color: "rgba(224,238,255,0.16)", lineHeight: 1.7 }}>
