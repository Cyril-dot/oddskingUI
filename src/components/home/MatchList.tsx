...
async function fetchAdminMatchOdds(matchId: string): Promise<unknown[]> {
  try {
    const raw = await fetch(
      `https://futballbackend-production-00d6.up.railway.app/api/public/admin-matches/${matchId}/odds`
    ).then((r) => r.json());
...
function SpecialGamesSection({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const raw = await fetch(
          'https://futballbackend-production-00d6.up.railway.app/api/public/admin-matches?ngrok-skip-browser-warning=true',
        ).then((r) => r.json());
        if (!alive()) return;
